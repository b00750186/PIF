...
double xSol_CPIF_SEC_2[MAX_SIZE];
double obj_value_CPIF_SEC_2;
...
/*****************************************************************/
{
	...
	for (int j = 0; j < inst->dimension_c; j++) {
		...
		for (auto i : inst->fstar_cover[j]) {
			...
			if (inst->I_tilde[j] == 1.0 && inst->Js[j]) {
...
void build_model_CPIF_NT(instance* inst) {
	...
	if (inst->status != 0)
	{
		printf("error in CPXnewcols 890\n");
		exit(-1);
	}
...
void solve_model_CPIF_NT(instance* inst) {
	...
	*/
	...
	CPXsetintparam(inst->env_CPIF, CPX_PARAM_MIPCBREDLP, CPX_OFF);        // let MIP callbacks work on the original model
	CPXsetintparam(inst->env_CPIF, CPX_PARAM_PRELINEAR, CPX_OFF);              // assure linear mappings between the presolved and original models
