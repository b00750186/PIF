...
int CPXPUBLIC mylazycallback_NZ(CPXCENVptr env, void* cbdata, int wherefrom, void* cbhandle, int* useraction_p) {
	...
	while (!remote_nodes.empty()) {
		...
		if (status != 0) {
			cout<< "error in CPXcutcallbackadd" << endl;
...
int CPXPUBLIC mylazycallback_NZ_min(CPXCENVptr env, void* cbdata, int wherefrom, void* cbhandle, int* useraction_p) {
	...
	*/
	...
	cout << "Node LP Lazy callback written" << endl;
	cin.get();
#endif
...
void add_singleflow_constraints_CPIF(instance* inst) {
	...
	inst->rmatbeg[0] = 0;
	inst->nzcnt = (inst->dimension - 1) * 2 + inst->dimension - 1;
	...
	for (int i = 1; i < inst->dimension; i++) {
		inst->rmatind[counter] = inst->f[i][0];
		inst->rmatval[counter] = -1.0;
		counter++;
	}
...
void build_model_CPIF_SZ(instance* inst)
{
	...
	inst->z = (int*)calloc(inst->dimension, sizeof(int));
	...
	if (inst->status != 0)
	{
		cout << "error in CPXnewcols 91" << endl;
		exit(-1);
	}
	...
	int** a;
	a = (int**)calloc(inst->dimension, sizeof(int*));
	for (int i = 0; i < inst->dimension; i++) { a[i] = (int*)calloc(inst->dimension, sizeof(int)); }
	...
	for (int k = 0; k < inst->dimension; k++) {
		...
		if (inst->status != 0)
		{
			cout << "error in CPXaddrows 286" << endl;
			exit(-1);
		}
	}
	...
	if (strncmp(inst->instance_type, "pmed", 4) == 0) {
		...
		if (inst->status != 0)
		{
			cout << "error in CPXaddrows 319" << endl;
			exit(-1);
		}
...
void build_model_CPIF_NZ(instance* inst)
{
	...
	//adding variables
	inst->ccnt = inst->dimension + inst->dimension;
	inst->obj = (double*)calloc(inst->ccnt, sizeof(double));
	inst->lb = (double*)calloc(inst->ccnt, sizeof(double));
	inst->ub = (double*)calloc(inst->ccnt, sizeof(double));
	inst->c_type = (char*)calloc(inst->ccnt, sizeof(char));
...
void build_model_CPIF_MZ(instance* inst)
{
	...
	//adding variables
	inst->ccnt = inst->dimension * inst->dimension * inst->dimension + inst->dimension + inst->dimension;
	inst->obj = (double*)calloc(inst->ccnt, sizeof(double));
	inst->lb = (double*)calloc(inst->ccnt, sizeof(double));
	inst->ub = (double*)calloc(inst->ccnt, sizeof(double));
	inst->c_type = (char*)calloc(inst->ccnt, sizeof(char));
	...
	for (int i = 0; i < inst->dimension; i++)
		for (int j = 0; j < inst->dimension; j++)
			for (int k:inst->fstar_facility[j])
				if (j != k && inst->c[j][k] <= inst->r)
			{
				inst->obj[counter] = 0.0;
				inst->lb[counter] = 0.0;
	...
	if (inst->status != 0)
	{
		cout << "error in CPXnewcols 435" << endl;
		exit(-1);
	}
	...
	cout << "Adding constraints coverage" << endl;
	// z_k <= sum a_ki y_i
	inst->rcnt = 1;
	inst->rhs = (double*)calloc(inst->rcnt, sizeof(double));
	inst->rhs[0] = 0.0;
	...
	cout << "Adding multiflow constraints" << endl;
...
void solve_model_CPIF(instance* inst) {
	...
	double fixed_obj = inst->dimension;
