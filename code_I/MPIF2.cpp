	...
	for (int j = 0; j < inst->dimension; j++) {
		for (int k = 0; k < inst->dimension; k++) {
			if (!inst->b[j][inst->index_sorted[j][k]]) {
				double lhs = 0.0;
				for (int i = k; i >= 0; i--) {
					lhs += (inst->c[j][inst->index_sorted[j][k]] - inst->c[j][inst->index_sorted[j][i]]) * xSol_v_2[inst->y[inst->index_sorted[j][i]]];
				}
				lhs += xSol_v_2[inst->v[j]];
				double rhs = inst->c[j][inst->index_sorted[j][k]];
				if (lhs < rhs - 0.00001) {
					int counter = 0;
					for (int i = k; i >= 0; i--) {
						rmatind_v_2[counter] = inst->y[inst->index_sorted[j][i]];
						rmatval_v_2[counter] = (inst->c[j][inst->index_sorted[j][k]] - inst->c[j][inst->index_sorted[j][i]]);
						counter++;
					}
					rmatind_v_2[counter] = inst->v[j];
					rmatval_v_2[counter] = 1.0;
					rhs_v_2 = inst->c[j][inst->index_sorted[j][k]];
	...
	if (inst->fstar_facility[0].size() > 1)
		for (auto second_facility : inst->fstar_facility[0])
			if (second_facility != 0)
			{
				double cost = 0.0;
				for (int i = 0; i < inst->dimension; i++) {
					cost += min(1.0 * inst->c[0][i],1.0* inst->c[second_facility][i]);
				}

				if (best_cost > cost) best_cost = cost;
			}

	best_cost += inst->fixed_cost;
...
void build_model_MPIF_NV(instance* inst) {
	...
	inst->v = (int*)calloc(inst->dimension, sizeof(int));
...
void solve_model_MPIF_NV(instance* inst) {
	...
	if (inst->status)
	{
		cout << "error in CPXsetuserconstraintcallbackfunc" << endl;
	}
