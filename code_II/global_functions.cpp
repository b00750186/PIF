...
void write_log_CPIF(instance* inst,double solution_time, int cut_connect , int cut_cover) {
	...
	fprintf(pFile, "%s\t %s \t %f \t %f \t %s \t %s \t %s \t %f \t %f \t %f \t %f \t %f \t %d \t %d \t %d \t %d \t %f \t lpstatus: %d\n", inst->instance_name, inst->instance_number.c_str(), inst->r, inst->R, inst->problem_type, inst->options, inst->benders, solution, inst->objval, inst->best_lb, inst->root_bound, inst->opened_facilities, inst->lazy_cuts_connectivity, inst->lazy_cuts_coverage, inst->user_cuts_coverage, nodecount, solution_time, lpstat);
	fclose(pFile);
...
void write_log_MPIF(instance* inst, double solution_time, int cut_connect, int cut_cost, int user_cut_cost) {
	...
	fprintf(pFile, "%s\t %s \t %f \t %s \t %s \t %s \t %f \t %f \t %f \t %f \t %f \t %d \t %d \t %d \t %d \t %f \t lpstatus: %d\n", inst->instance_name, inst->instance_number.c_str(), inst->r, inst->problem_type, inst->options, inst->benders, solution, inst->objval, inst->best_lb, inst->root_bound,inst->opened_facilities, inst->lazy_cuts_connectivity, inst->lazy_cuts_cost, inst->user_cuts_cost, nodecount, solution_time, lpstat);
	fclose(pFile);
#endif
}
